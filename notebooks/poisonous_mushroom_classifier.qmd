...
```{python}
 # Data Validation Check: Anomalous correlation between features

# create list of all pairwise feature combinations 
...
cramers_matrix = pd.DataFrame(np.eye(len(feature_cols)), columns=feature_cols, index=feature_cols) 

for feature1, feature2 in pairs:
    v = cramers_v(X_train, feature1, feature2)
